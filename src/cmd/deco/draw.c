...
static int longwid(void);
static int briefcols(void);
static int briefwid(void);
static int briefoff(int index);
static int briefsep(int index);
static char *filnam(const char *p, int tail);
static char *filnam_wide(const char *p);

static int longwid(void)
{
    return (widewin ? (PAGEWID - 3) / 2 : PAGEWID - 2);
}
...
static int briefwid(void)
{
    ...
    return ((PAGEWID - (ncols + 1)) / ncols);
}

static int briefoff(int index)
{
    return (1 + index * (briefwid() + 1));
}

static int briefsep(int index)
{
    return (briefoff(index) - 1);
}
...
void drawbanners()
{
    ...
    if (tty)
        VPutString(tty);
    if (user)
        VMPutString(0, COLS / 2 - 1 - strlen(user), user);
    VPutChar('.');
    if (group)
        VPutString(group);
...
static void draw1dir(struct dir *c) /* redraw window */
{
    ...
    if (widewin)
        switch (c->view) {
        case VIEW_BRIEF:
            for (i = 1; i < briefcols(); ++i)
                VCorner(1, briefsep(i), 2);
            break;
        case VIEW_LONG:
        case VIEW_WIDE:
            VCorner(1, longwid() + 1, 2);
            break;
        }
    else if (c->view == VIEW_BRIEF)
        VCorner(H + 2, BASECOL(c) + briefsep(1), 8);
    ...
    case VIEW_LONG:
    case VIEW_WIDE:
        VClearBox(2, BASECOL(c) + 1, H, longwid());
        drawpage(BASECOL(c) + 1, &c->cat[c->topfile], len, c);
        if (widewin) {
            VClearBox(2, BASECOL(c) + longwid() + 2, H, longwid());
            if (len > H)
                drawpage(BASECOL(c) + longwid() + 2, &c->cat[c->topfile + H], len - H, c);
        }
        break;
    case VIEW_BRIEF:
        for (i = 0; i < briefcols(); ++i) {
            VClearBox(2, BASECOL(c) + briefoff(i), H, briefwid());
            if (len > i * H)
                drawpage(BASECOL(c) + briefoff(i),
                    &c->cat[c->topfile + i * H], len - i * H, c);
        }
        break;
    }
...
static void drawframe(int c, int view, int stawin)
{
    ...
    if (stawin) {
        ...
        case VIEW_LONG:
            VHorLine(H + 2, 1, PAGEWID - 2);
            VVertLine(0, 2, H + 2);
            VVertLine(longwid() + 1, 2, H);
            VVertLine(PAGEWID - 1, 2, H + 2);
            VCorner(1, 0, 1);
            VCorner(1, PAGEWID - 1, 3);
            VCorner(H + 2, 0, 4);
            VCorner(H + 2, longwid() + 1, 8);
            VCorner(H + 2, PAGEWID - 1, 6);
            VCorner(H + 4, 0, 7);
            VCorner(H + 4, PAGEWID - 1, 9);
            break;
        case VIEW_BRIEF:
            VHorLine(H + 2, 1, PAGEWID - 2);
            VVertLine(0, 2, H + 2);
            for (c = 1; c < briefcols(); ++c)
                VVertLine(briefsep(c), 2, H);
            VVertLine(PAGEWID - 1, 2, H + 2);
            VCorner(1, 0, 1);
            VCorner(1, PAGEWID - 1, 3);
            VCorner(H + 2, 0, 4);
            for (c = 1; c < briefcols(); ++c)
                VCorner(H + 2, briefsep(c), 8);
            VCorner(H + 2, PAGEWID - 1, 6);
            VCorner(H + 4, 0, 7);
            VCorner(H + 4, PAGEWID - 1, 9);
            break;
        ...
        VVertLine(c, 2, H + 2);
        if (view == VIEW_BRIEF)
            VVertLine(c + briefsep(1), 2, H);
        VVertLine(c + PAGEWID - 1, 2, H + 2);
        VCorner(1, c, 1);
        VCorner(1, c + PAGEWID - 1, 3);
        VCorner(H + 2, c, 4);
        VCorner(H + 2, c + PAGEWID - 1, 6);
        VCorner(H + 4, c, 7);
        if (view == VIEW_BRIEF)
            VCorner(H + 2, c + briefsep(1), 8);
        VCorner(H + 4, c + PAGEWID - 1, 9);
    }
    VSetNormal();
}
...
static void drawcwd(int col, char *wd)
{
    ...
    if (widewin && col)
        col = longwid() + 1;
    VMPutString(1, col + (PAGEWID - len) / 2, wd);
}

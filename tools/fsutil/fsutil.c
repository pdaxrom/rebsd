 ...
 * Create a directory.
 */
int add_directory (fs_t *fs, char *name, int mode, int owner, int group)
{
 ...
 */
int add_device (fs_t *fs, char *name, int mode, int owner, int group,
 ...
 */
int add_file (fs_t *fs, const char *path, const char *dirname,
 ...
 * Create a symlink.
 */
int add_symlink (fs_t *fs, const char *path, const char *link,
    int mode, int owner, int group)
{
    fs_file_t file;
    int len, ok;
 ...
 * Create a hard link.
 */
int add_hardlink (fs_t *fs, const char *path, const char *link)
{
 ...
 */
int add_object (fs_t *fs, char *name)
{
    int majr, minr;
    char type;
    char *p;
    ...
    if (p && p[1] == 0) {
        *p = 0;
        return add_directory (fs, name, 0777, 0, 0);
    }
    p = strrchr (name, '!');
    if (p) {
        *p++ = 0;
        ...
        return add_device (fs, name, 0666, 0, 0, type, majr, minr);
    }
    return add_file (fs, name, 0, -1, 0, 0);
}
 ...
 */
int add_contents (fs_t *fs, const char *dirname, const char *manifest)
{
    manifest_t m;
    void *cursor;
    char *path, *link;
        ...
        &owner, &group, &majr, &minr)) != 0)
    {
        switch (filetype) {
        case 'd':
            if (add_directory (fs, path, mode, owner, group))
                ndirs++;
            else
                failures++;
            break;
        case 'f':
            if (add_file (fs, path, dirname, mode, owner, group))
                nfiles++;
            else
                failures++;
            break;
        case 'l':
            if (add_hardlink (fs, path, link))
                nlinks++;
            else
                failures++;
            break;
        case 's':
            if (add_symlink (fs, path, link, mode, owner, group))
                nsymlinks++;
            else
                failures++;
            break;
        case 'b':
            if (add_device (fs, path, mode, owner, group, 'b', majr, minr))
                ndevs++;
            else
                failures++;
            break;
        case 'c':
            if (add_device (fs, path, mode, owner, group, 'c', majr, minr))
                ndevs++;
            else
                failures++;
            break;
...
int main (int argc, char **argv)
{
    ...
    if (newfs) {
        ...
        if (i == argc-2) {
            /* Add the contents from the specified directory.
             * Use the optional manifest file. */
            if (! add_contents (&fs, argv[i+1], manifest)) {
                fs_close (&fs);
                return -1;
            }
        }

...
`N64_USERLAND_COMPILER` exists as the future userland compiler selector. Its
default is `gcc`. `pcc` is intentionally rejected until the new PortableCC port
passes the required QEMU Malta tests.

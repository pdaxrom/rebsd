...
- `ct_ticks`, VM accounting storage и генератор `vers.c` перенесены к общим
  архитектурно-нейтральным владельцам; MIPS-платы используют те же владельцы;
- позднее `pipedev` полностью удалён: anonymous `pipe(2)` использует общий
  in-core buffer и не зависит от файловой системы;
...
через обычный `dd`. Последующая DMA-загрузка выявила гонку между ATA IRQ14 и
защёлкиванием bus-master interrupt status на VIA. Общий handler исправлен и
host regression воспроизводит ранний ATA IRQ без BM interrupt bit. Следующий
аппаратный запуск повторяет `ata=dma`, затем проверяет `ata=auto` с
сохранением полного mode/error log. Разрешение записи на настоящий CF
является отдельным
...
IDE attachment подключён к уже существующему common disk read-ahead, который
ранее использовался USB mass storage, но оставался выключенным для IDE.
Размер окна равен максимальной команде PCI IDE: 256 секторов/128 KiB. PRDT
содержит два 64-KiB региона и соблюдает запрет PCI IDE на пересечение каждым
регионом 64-KiB boundary. Поэтому
последовательное чтение блочного `/dev/sd0` больше не запускает отдельную
DMA-команду на каждый 1-KiB buffer-cache block.

Стандартный общий raw-disk character path, уже используемый Ci20/Malta,
подключён и на i686 как `/dev/rsd0`--`/dev/rsd0p`. Он использует
...
немедленно вызывает `netintr()` только когда обработчики текущего IRQ изменили
`netisr`; ранее ожидающая работа выполняется на system-call или timer return.
Поэтому IRQ1/IRQ12 не исполняют чужой сетевой backlog, а IRQ сетевой карты
обрабатывает поставленную им работу без timer delay.

...
def main() -> None:
    ...
    ide_clock_host_start: float | None = None
    ide_clock_host_elapsed: float | None = None
    completed = False
    ...
    while time.monotonic() < deadline:
        ...
        if (
            ...
            process.stdin.flush()
            if commands[command_index] == IDE_CLOCK_COMMAND:
                ide_clock_host_start = time.monotonic()
            command_sent = True
        if (
            command_sent
            ...
            if commands[command_index] == IDE_CLOCK_COMMAND:
                assert ide_clock_host_start is not None
                ide_clock_host_elapsed = (
                    time.monotonic() - ide_clock_host_start
                )
            command_index += 1
            command_sent = False
    ...
    if args.disk is not None and args.ata_mode in ("pio", "dma"):
        ide_clock_match = re.search(
            ...
            r"([0-9]+\.[0-9]) real.*?"
            r"REBSD_I686_IDE_CLOCK_END",
            output,
            re.DOTALL,
        )
        if ide_clock_match is None or ide_clock_host_elapsed is None:
            raise SystemExit("qemu-boot-smoke: missing IDE clock result")
        ide_clock_guest_elapsed = float(ide_clock_match.group(1))
        if ide_clock_guest_elapsed + 0.15 < ide_clock_host_elapsed * 0.5:
            raise SystemExit(
                "qemu-boot-smoke: PIT clock lost during IDE syscall: "
                f"guest={ide_clock_guest_elapsed:.1f}s "
                f"host={ide_clock_host_elapsed:.3f}s"
            )

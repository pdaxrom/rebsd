...
static int
i386_usb_pci_prepare_controller(struct i386_usb_pci_controller *controller,
...
i386_usb_pci_prepare(void)
{
    ...
    ohci_error = i386_usb_pci_prepare_controller(&i386_ohci_pci,
        "ohci0", I386_PCI_INTERFACE_OHCI);
    if (ohci_error != 0 && ohci_error != ENXIO)
        return ohci_error;
    ehci_error = i386_usb_pci_prepare_controller(&i386_ehci_pci,
        "ehci0", I386_PCI_INTERFACE_EHCI);
    if (ehci_error != 0 && ehci_error != ENXIO)
        return ehci_error;
    if (ohci_error == ENXIO && ehci_error == ENXIO)
        return ENXIO;
    return 0;
}

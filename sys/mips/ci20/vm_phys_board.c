...
static int
ci20_vm_reserve_present(struct vm_phys_map *map, vm_size_t ram_size,
...
vm_phys_board_register(struct vm_phys_map *map, vm_size_t ram_size)
{
    ...
    if (ram_size != CI20_RAM_SIZE)
        return EINVAL;
    error = vm_phys_map_add_ram(map, CI20_PHYS_RAM_BASE, ram_size, "ram");
    if (error != 0)
        return error;
    ...
    if (error != 0)
        return error;
    uarea_start = MIPS_KSEG_TO_PHYS(CI20_U0AREA_VADDR);
    ...
    error = ci20_vm_reserve_present(map, ram_size,
        CI20_RAMDISK_VAR_PHYS_START, CI20_RAMDISK_VAR_BYTES,
        "/var ramdisk");
    if (error != 0)
        return error;
    error = ci20_vm_reserve_present(map, ram_size,
    ...
    if (error != 0)
        return error;
    return ci20_vm_reserve_present(map, ram_size,
